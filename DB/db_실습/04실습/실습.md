...
```sql
SELECT Country AS 나라, COUNT(Country) AS '고객 수'
FROM customers
GROUP BY 나라
ORDER BY (SELECT COUNT(Country)) DESC
LIMIT 5;
```
...
FROM albums
GROUP BY ArtistId
ORDER BY (SELECT COUNT(ArtistId)) DESC
LIMIT 1;
```
...
FROM albums 
GROUP BY ArtistId
HAVING (SELECT COUNT(ArtistId)) >= 10
ORDER BY (SELECT COUNT(ArtistId)) DESC;
```
